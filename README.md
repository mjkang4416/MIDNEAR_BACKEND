...
- 관리자 콘솔 다중 조건 검색 시 쿼리 분기 증가로 유지보수가 어려워 MyBatis 동적 쿼리 구조로 리팩토링하여 검색 안정성 개선
- 주문/환불/포인트/쿠폰 처리 로직을 하나의 트랜잭션으로 묶어 중간 실패 시 롤백되도록 설계하여 데이터 정합성 강화
- Jenkins 기반 CI/CD 파이프라인 구축으로 수동 배포 과정을 자동화하여 배포 리드타임 단축
